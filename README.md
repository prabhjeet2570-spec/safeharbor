...
SafeHarbor intercepts HTTPS traffic to AI services (ChatGPT, Claude, Gemini), detects Protected Health Information (PHI) using NLP, redacts it in real-time, and provides a cybersecurity-themed governance dashboard with live WebSocket updates and automated voice alerts.
...
# Install the proxy-side dependencies (first run only)
pip install -r requirements.txt
python -m spacy download en_core_web_lg

# Run mitmproxy with the SafeHarbor addon
mitmproxy -s safeharbor_addon.py
```
...
When a high-risk PHI exposure is detected (severity critical/high, risk score >= 70), SafeHarbor can automatically call the responsible staff via VAPI to notify them.
...
|----------|----------|---------|-------------|
| `DATABASE_URL` | Yes (Docker sets it) | `postgresql://safeharbor:safeharbor@localhost:5432/safeharbor` | PostgreSQL connection string |
...
```
SafeHarbor/
├── safeharbor_addon.py    # mitmproxy addon — intercepts, detects PHI, posts to backend
├── phi_redactor.py         # PHI detection engine (Presidio + regex fallback)
├── requirements.txt        # Proxy-side deps (mitmproxy, Presidio, spaCy)
